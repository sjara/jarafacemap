...
istr = ['pupil', 'motSVD', 'blink', 'running', 'pixelchange']
...
class MainW(QtGui.QMainWindow):
    def __init__(self, moviefile=None, savedir=None):
        ...
        self.p1.setMouseEnabled(x=True,y=True)  # y=False
        ...
        self.p2.setMouseEnabled(x=True,y=True)  # y=False
    ...
    def make_buttons(self):
        binLabel = QtGui.QLabel("pixelchange smooth win:")
        binLabel.setStyleSheet("color: gray;")
        self.smoothWin = QtGui.QLineEdit()
        self.smoothWinLenght = 10
        self.smoothWin.setText(str(self.smoothWinLenght))
        self.smoothWin.setEnabled(False)
        self.smoothWin.setFixedWidth(45)
        self.l0.addWidget(binLabel, 20, 0, 1, 3)
        self.l0.addWidget(self.smoothWin, 21, 0, 1, 3)
        #self.smoothWin.returnPressed.connect(self.pupil_sigma_change)
        ...
        self.comboBox.addItem("pixelchange")
        self.newROI = 0
    ...
    def process_ROIs(self):
        ...
        if len(self.save_path) > 0:
            ...
            savepath = None
        print(savepath)
        from importlib import reload; reload(process) ### sjara DEBUG
    ...
    def plot_trace(self, wplot, proctype, wroi, color):
        ...
        if proctype==0 or proctype==2:
            ...
            pixdelta = self.pixelchange[wroi]
            #pixdelta *= np.sign(pixdelta.mean(axis=0))
            pixdelta -= pixdelta.min()
            pixdelta /= pixdelta.max()
            pixdelta *= 8
            pixdelta -= 4
            wp.plot(pixdelta, pen=color)
            #winsize = self.ops['pixelschange_smooth_win']
            winsize = self.smoothWinLenght
            smoothpixdelta = np.convolve(pixdelta, np.ones(winsize), mode='same')/winsize
            wp.plot(smoothpixdelta, pen='w')
            #tr = pixdelta
            tr = smoothpixdelta
        return tr
